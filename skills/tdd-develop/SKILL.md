...
You do not implement tickets yourself. That is `tdd-implement`'s job, one ticket at a time, so that the same discipline runs whether the work is sequential or dispatched to parallel workers.
...
If `tdd-plan` is not installed, say so plainly and stop rather than improvising a plan. The board's structure is what everything downstream depends on.
...
For each ticket, use the installed `tdd-implement` skill with **exactly that one ticket**, and give it the complete packet: ticket path, board path, approved seam, write scope, verification commands, baseline, and `isolation: shared working tree`.
...
- **done** — check the evidence per the rules in `tdd-implement`'s execution report reference. Count red-green pairs against acceptance criteria. Missing or inconsistent evidence downgrades the ticket to `unverified`.
...
Switch to `tdd-develop-parallel` when the board has several unblocked tickets with disjoint write ownership and the user wants them run concurrently. Do not switch when the board is a linear chain, when the repository is not under git, or when the test suite cannot tolerate concurrent runs — in those cases parallelism costs more than it returns.
